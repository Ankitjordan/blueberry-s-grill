...
export const RetroHeader: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // passive: true — browser can skip blocking check, improves scroll jank
                      ...
                      {cat.items.map((item) => (
                        <li key={item.name}>
                          <Link
                            href={item.href}
                            onClick={closeMobileMenu}
                            className="w-full flex items-center justify-between py-1.5 border-b border-slate-200 hover:text-[#D4AF37] transition-colors"
                          >
