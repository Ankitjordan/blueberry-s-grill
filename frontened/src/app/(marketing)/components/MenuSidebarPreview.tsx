...
const StarRating: React.FC<{ rating: number }> = ({ rating }) => (
  <div className="flex items-center gap-0.5">
    ...
    <span className="text-xs font-bold text-gray-400 ml-1">{rating}.0 / 5</span>
  </div>
);
...
export const DesktopSidebarPreview: React.FC<DesktopSidebarProps> = ({ item }) => (
  <div
    className={`w-72 flex-shrink-0 border-l-2 border-black overflow-hidden transition-all duration-300 ease-in-out ${
      item ? "opacity-100 translate-x-0" : "opacity-0 translate-x-6 pointer-events-none"
    }`}
  >
    {item && (
      <div className="h-full flex flex-col bg-[#1a1a1a] text-white">
        <div className="relative w-full h-48 flex-shrink-0">
          <Image src={item.image} alt={item.name} fill sizes="288px" className="object-cover" />
...
export const MobileSidebarPreview: React.FC<MobileSidebarProps> = ({ item, onClose }) => (
  <div
    ...
    {item && (
      <>
        ...
        <div className="flex flex-col flex-1 px-8 py-6 overflow-y-auto">
          ...
          <h2 className="text-3xl font-black uppercase tracking-tighter mb-3">{item.name}</h2>
          <StarRating rating={item.rating} />
          <p className="text-gray-400 mt-4 text-sm leading-relaxed">{item.description}</p>
          <div className="mt-6">
            <span className="text-4xl font-black font-serif italic text-[#d4ff00]">{item.price}</span>
          </div>
