...
import type { DessertItem } from "../../desserts/data";

interface NameColumnProps {
	name: string;
}

const NameColumn = ({ name }: NameColumnProps) => {
	return (
...
interface RatingTagsColumnProps {
	rating?: number;
	tags?: string[];
}

const RatingTagsColumn = ({ rating, tags }: RatingTagsColumnProps) => {
	if (!rating && !tags) return null;
	return (
		<motion.div
			initial={{ opacity: 0, x: 20 }}
			animate={{ opacity: 1, x: 0 }}
...
export function MenuInfoRating({ name, rating, tags }: DessertItem) {
	return (
		<div className="flex flex-col lg:flex-row justify-between items-start gap-12 px-2 relative">
			...
			<RatingTagsColumn rating={rating} tags={tags} />
		</div>
	);
}
