...
[Persistent sandboxes](/docs/vercel-sandbox/concepts/persistent-sandboxes) are available in beta. Persistent sandboxes automatically save state when stopped and restore it when resumed, removing the need to manage snapshots manually.
...
## [Sandboxes vs containers](#sandboxes-vs-containers)
...
sandbox exec sb_abc123 npm install

# Run interactively
sandbox exec --interactive --tty sb_abc123 bash

# Run with environment variables
sandbox exec --env DEBUG=true sb_abc123 npm test
```
...
```
# Blocking: waits for the command to finish
result = sandbox.run_command('npm', ['install'])
print(result.exit_code)

# Detached: returns immediately
cmd = sandbox.run_command_detached('npm', ['run', 'dev'])
...
### [When not to use sandboxes](#when-not-to-use-sandboxes)
...
* [Persistent sandboxes (Beta)](/docs/vercel-sandbox/concepts/persistent-sandboxes): Sandboxes that auto-save state and resume where you left off.
...
Quickstart](/docs/vercel-sandbox/quickstart)[Next
