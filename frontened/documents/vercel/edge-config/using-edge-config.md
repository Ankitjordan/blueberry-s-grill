...
Vercel shows the full token value only once, right after creation, whether
...
Copy the token from the response immediately and store it somewhere safe.
...
Append the `teamId` query parameter to the request if the
...
Getting Started](/docs/edge-config/get-started)[Next
