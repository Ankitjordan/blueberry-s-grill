...
Those with the [owner](/docs/rbac/access-roles#owner-role), [member](/docs/rbac/access-roles#member-role) and [admin](/docs/rbac/access-roles#admin-role) roles can manage Trusted Sources
...
### [Forward the OIDC token from your code](#forward-the-oidc-token-from-your-code)
...
app/api/call-protected/route.ts
...
### [Customize this project's self-access](#customize-this-project's-self-access)
...
Saved rules replace the defaults. If you still want production to reach production, preview to reach preview, or development to reach preview for CLI access, keep those rows in the editor.

To return to the default, open the menu on the same row and select Reset to default.

### [Add another Vercel project](#add-another-vercel-project)
...
### [Remove a Vercel project](#remove-a-vercel-project)
...
### [Add a GitHub Actions service](#add-a-github-actions-service)
...
### [Add another external service](#add-another-external-service)
...
### [How claim matching works](#how-claim-matching-works)
...
### [Attach the OIDC token from your code](#attach-the-oidc-token-from-your-code)
...
Bitbucket only publishes the OIDC discovery document after Pipelines OIDC is enabled in the workspace and at least one pipeline has run. If Vercel can't fetch the discovery document when you add the trusted source, run a pipeline once and try again.
...
### [Remove an external service](#remove-an-external-service)
