...
## [Why automated systems get blocked](#why-automated-systems-get-blocked)
...
## [Choose the right bypass method](#choose-the-right-bypass-method)
...
## [Set up Protection Bypass for Automation](#set-up-protection-bypass-for-automation)

1. ### [Enable Protection Bypass and create a secret](#enable-protection-bypass-and-create-a-secret)
...
2. ### [Pass the secret with each request](#pass-the-secret-with-each-request)
   ...
   When using the query parameter method, the secret appears in the URL. URLs are
   often logged by proxies, CDNs, and server access logs. Prefer the header
   method when your tool supports custom headers.
3. ### [Store the secret securely](#store-the-secret-securely)
...
## [Set up Deployment Protection Exceptions](#set-up-deployment-protection-exceptions)
...
Deployment Protection Exceptions make the specified domain accessible to
...
## [Why Trusted IPs may not work for automation](#why-trusted-ips-may-not-work-for-automation)
...
### [AI agent calling a preview API route](#ai-agent-calling-a-preview-api-route)
...
### [MCP server accessing a protected deployment](#mcp-server-accessing-a-protected-deployment)
...
### [Webhook receivers on protected deployments](#webhook-receivers-on-protected-deployments)
...
The bypass secret is visible in the URL when using the query parameter method.
...
### [Testing with the Vercel CLI](#testing-with-the-vercel-cli)
...
## [Manage bypass secrets safely](#manage-bypass-secrets-safely)
...
### [Automated requests still receive a login page or 403 response](#automated-requests-still-receive-a-login-page-or-403-response)
...
### [Secret mismatch after regeneration](#secret-mismatch-after-regeneration)
...
### [In-browser tests fail on navigation after the initial page load](#in-browser-tests-fail-on-navigation-after-the-initial-page-load)
...
### [Dynamic IP addresses prevent Trusted IPs from working](#dynamic-ip-addresses-prevent-trusted-ips-from-working)
...
## [Agents that need browser interaction](#agents-that-need-browser-interaction)
